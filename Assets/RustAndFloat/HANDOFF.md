...
- **広大な白砂ビーチと透明な楽園の海**: 標高6.0m〜8.5m、島を一周する幅約50mの鮮やかな白砂ビーチテラス。
  ...
  - **空中浮遊・滑空（グライド）の操作感改善**:
    - 気持ちいい滑空感をそのまま保ちつつ、方向修正レスポンス向上（YawRate 180°/s、カメラ追従アシスト、微小左右スライド）と、Sキーによるふわり上昇（+1.75m/s）、Wキーによるダイブ降下（-3.5m/s）を実装。
    - 空中滑空中に専用操作ガイドUI（`【A / D】方向修正 【S】ふわり上昇 【W】ダイブ降下 【Space長押し】滑空`）が動的に表示される。
